...
export function ElevatorSummaryCards(): React.JSX.Element {
  ...
  return (
    <section className="grid gap-4 md:grid-cols-3" aria-label="elevator-summary-cards">
      {cards.map((card) => (
        <article
          key={card.label}
          className="rounded-2xl border border-white/10 bg-slate-950/40 px-4 py-4 shadow-[0_12px_40px_rgba(0,0,0,0.2)]"
        >
